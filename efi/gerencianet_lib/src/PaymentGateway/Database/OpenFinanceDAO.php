...
class OpenFinanceDAO
{
    /**
     * Cria um novo registro de pix na tabela tblefiopenfinance.
     *
    ...
    {
        try {
            return Capsule::table('tblefiopenfinance')->insert($data);
        } catch (\Throwable $th) {
    ...
    {
        try {
            return Capsule::table('tblefiopenfinance')->where('invoiceid', $invoiceid)->first();
        } catch (\Throwable $th) {
            TransactionLogger::log(json_encode($th->getMessage()), TransactionLogger::ERROR_LOG);
            return null;
        }
    ...
    /**
     * Atualiza um registro de pix pelo invoiceid.
     *
     * @param int $invoiceid ID da cobrança (invoiceid).
     * @param array $data Dados atualizados para o pix via OpenFinance.
     * @return bool Sucesso ou falha na atualização.
     */
    public static function update(int $invoiceid, array $data): bool
    {
        try {
            return Capsule::table('tblefiopenfinance')

...
require_once BASE_DIR . 'modules/gateways/efi/gerencianet_lib/models/invoice/InvoiceCreationProcessor.php';
require_once BASE_DIR . 'modules/gateways/efi/gerencianet_lib/models/cron/PreCronProcessor.php';
include BASE_DIR . 'modules/gateways/efi/gerencianet_lib/Gerencianet_WHMCS_Interface.php';
include_once BASE_DIR . 'modules/gateways/efi/gerencianet_lib/GerencianetIntegration.php';
...
function handlerInvoiceCreation($vars) {
    try {
       
         $invoiceCreation = new InvoiceCreationProcessor();
         
         $invoiceCreation->handler($vars);
    } catch (Exception $e) {
        logActivity("Erro ao processar hook de criação de fatura: " . $e->getMessage());

        $logFile = __DIR__ . '/error_log.txt';  // Arquivo de log
        $currentTime = date('Y-m-d H:i:s');
        $logContent = "[$currentTime] Erro ao processar hook de criação de fatura: " . $e->getMessage() . "\n";

        // Adicionar mensagem ao arquivo de log
...
function handlerPreCronJob($vars){
    $preCronProcessor = new PreCronProcessor();
    $preCronProcessor->handler($vars);
}
add_hook('InvoiceCancelled', 1, 'gerencianetCancelCharge');
add_hook('UpdateInvoiceTotal', 1, 'gerencianetUpdateBillet');
add_hook('InvoiceCreation', 1, 'handlerInvoiceCreation');
add_hook('PreCronJob', 1, 'handlerPreCronJob');
